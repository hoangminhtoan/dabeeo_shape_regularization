...
def regularize_segmentations(img_folder, seg_folder, out_folder, in_mode="semantic", out_mode="instance", samples=None):
	...
	for num, (satellite_image_file, building_segmentation_file) in enumerate(zip(img_files, seg_files)):
		...
		M = cv2.imread(building_segmentation_file, 0) # M should be 1-channel image
		M = np.uint16(M)
		...
		if samples is not None:
			...
			cv2.putText(R, "Suwon dataset, " + rgb_name + ", regularization", 
			...
			cv2.putText(M, "Suwon dataset, " + rgb_name + ", segmentation", 
				bottomLeftCornerOfText, 
				font, 
				fontScale,
				fontColor,
...
if __name__ == '__main__':
	regularize_segmentations(img_folder=var.INF_RGB, seg_folder=var.INF_SEG, 
							 out_folder=var.INF_OUT, 
							 in_mode="semantic", out_mode="instance", samples=None)
